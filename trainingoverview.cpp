...
void TrainingOverview::recalculateRow(int row)
{
    if(ui->trainingTable->item(row, 5) != NULL && ui->trainingTable->item(row, 6) != NULL){
        ...
        ui->trainingTable->item(row,7)->setData(Qt::EditRole, ui->trainingTable->item(row, 5)->data(Qt::EditRole).toDate().addYears(ui->trainingTable->item(row, 6)->data(Qt::EditRole).toInt()));
